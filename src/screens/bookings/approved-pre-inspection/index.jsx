...
import FinishBooking from '../../../components/bookings/pre-inspection/single-booking';

import { useIsFocused } from '@react-navigation/native';



const ApprovedPreInspection = (props) => {
...
useEffect(() => {
  ...
  const fetchBookingsDetails = async () => {
    try {
      ...
      let response = await Apis.HttpGetRequest(
        Constant.BASE_URL + Constant.GET_USER_BOOKINGS_DETAILS  + props.route?.params?.booking?._id,
        token
      );
      if (response?.status) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#edeeec' }}>
      <Header title="Approve Services/Spare" />
...
export default ApprovedPreInspection;