...
const Stepper = (props) => {
  ...
  const fetchGarageData = async () => {
    try {
      setLoading(true);
      let response = await Apis.HttpGetRequest(Constant.BASE_URL + Constant.AUTH.GURAGE_DEATIL_API, token)
      setLoading(false);
      if (response ?.status) {
  ...
  const addBooking = async () => {
    ...
    for (let index = 0; index < selectedAddOns.length; index++) {
      accessories.push(selectedAddOns[index]._id)
    }
  ...
  const renderContent = () => {
    switch (currentStep) {
      case 1:
        return <Step1 validateStep1={(isValid) => { setIsStep1Valid(isValid) }} setSelectedBike={setSelectedBike} {...props} />;
      case 2:
        return <Step2 calculateTotalAmount={() => calculateTotalAmount()} services={services} addOns={addOns} accessories={accessories}
    ...
    <Box p={0} flex={1} bg="#fff">
      ...
      <Box m={3}>
        <View style={styles.stepsContainer}>
          {["Select Bike", "Service", "Slot & Address", "Finalize"].map((data, step) => (
      ...
      {!isNotServicable && <View style={styles.controlsContainer}>
        ...
        <VStack style={{ position: "absolute", left: 50, alignSelf: "center" }} space={0} mb={0}>
