...
const CustomBottomSheet = forwardRef((props, ref) => {
    const {
        ...
        addOnData, setSelectedAddOns, closeBottomSheet, setData, setSelectedAccessories, accessories,setSelectedAddress,fetchSavedVechiclesData,selectedBike
    } = props;
    ...
    const renderContent = () => {
        switch (contentType) {
            case 'addons':
                return (<SelectAddOns setData={setData} data={addOnData.filter( d => d.cc._id === selectedBike)} setSelectedAddOns={setSelectedAddOns} closeBottomSheet={closeBottomSheet} />)
            case 'accessories':
